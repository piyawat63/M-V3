...
#define POL_TX_SIZE     4          /* ต้องตรงกับ POL_RX_SIZE ฝั่งรับ */
#define POL_TIMEOUT_MS  1000

typedef enum { POL_NEGATIVE = 0, POL_POSITIVE = 1 } polarity_t;

uint8_t polTxBuf[POL_TX_SIZE];
uint8_t polarity = POL_POSITIVE;   /* global — ยังไม่คิดวิธีหา */
...
void send_polarity(uint8_t pol);
/* USER CODE END PFP */
...
void send_polarity(uint8_t pol)
{
    ...
    polTxBuf[1] = 0;
    polTxBuf[2] = 0;
    polTxBuf[3] = (uint8_t)(pol ^ 0xA5);      /* ไบต์ตรวจสอบง่าย ๆ */

    if (HAL_SPI_Transmit(&hspi1, polTxBuf, POL_TX_SIZE, POL_TIMEOUT_MS) != HAL_OK)
    {
        HAL_SPI_Abort(&hspi1);
    }
}
...
void find_max_min(){
	...
	polarity = POL_POSITIVE;  //ยังไม่รู้วิธีหา
	count = 0;
	p_sample = ordered;;
